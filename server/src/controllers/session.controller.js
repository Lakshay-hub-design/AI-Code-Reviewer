...
export const previewSession = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to preview session", error: err.message });
  }
};
...
export const updateSession = async (req, res) => {
  try {
    ...
    if (session.owner.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Only the owner can update this session" });
    }
...
export const deleteSession = async (req, res) => {
  try {
    ...
    if (session.owner.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Only the owner can delete this session" });
    }
...
export const joinSession = async (req, res) => {
  try {
    ...
    if (!session) return res.status(404).json({ message: "Session not found" });
    if (!session.isPublic)
      return res.status(403).json({ message: "Session is private" });
    ...
    res
      .status(200)
      .json({ message: "Joined session succesfully", session: updatedSession });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Failed to join session", error: err.message });
  }
};
