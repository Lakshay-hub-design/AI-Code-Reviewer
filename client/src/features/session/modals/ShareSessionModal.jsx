    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#0F0F11] border border-zinc-800 rounded-2xl p-6 shadow-2xl">
        ...
        <p className="text-zinc-400 text-sm mb-5 truncate">
          ...
          <span className="text-zinc-300 font-medium">{initialSession.title}</span>
        </p>
        ...
        <div
          ...
          style={{
            background: isPublic ? 'rgba(34,197,94,0.05)' : 'rgba(255,255,255,0.03)',
        ...
        <div className="flex items-center gap-2 text-zinc-500 text-xs mb-5">
          <Users size={13} />
          <span>{initialSession.members?.length || 1} member{(initialSession.members?.length || 1) !== 1 ? 's' : ''} currently in this session</span>
        </div>
        ...
        {!isPublic ? (
          <p className="text-amber-500/70 text-xs flex items-center gap-1.5 mt-2">
            <Lock size={11} />
            Enable Public to share this link with others
          </p>
        ) : (
        ...
        <div className="mt-5 p-4 rounded-xl bg-zinc-900/50 border border-zinc-800/60">
          <p className="text-zinc-500 text-xs font-medium mb-2 uppercase tracking-wider">How joining works</p>
          <div className="space-y-1.5">
            {[
              'Teammate opens the join link',
              'They see a preview of your session',
              'They click "Join Session" (GitHub login required)',
              'They\'re added as a member and enter the editor',
            ].map((step, i) => (
              <div key={i} className="flex items-start gap-2 text-zinc-500 text-xs">
