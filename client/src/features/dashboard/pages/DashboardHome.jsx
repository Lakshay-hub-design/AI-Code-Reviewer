...
const DashboardHome = () => {
  ...
  const { list: sessions, loading, error } = useSelector((state) => state.session)
  useEffect(() => {
          ...
          <section className="mt-7">
            ...
            {!loading && !error && mySessions.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-zinc-500 text-sm">Create your first session to start collaborating</p>
              </div>
            )}
            ...
            {!loading && mySessions.length > 0 && (
              <div className="grid grid-cols-4 gap-6">
                {mySessions.map((session) => (
                  <SessionCard key={session._id} project={session} />
                ))}
              </div>
            )}
          </section>
