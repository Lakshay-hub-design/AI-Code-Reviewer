...
import { previewSession, joinSession } from "../../session/sessionSlice";
...
const JoinSessionModal = ({ onClose }) => {
  ...
  const extractId = (val) => {
    ...
    if (trimmed.includes("/join/"))
      return trimmed.split("/join/").pop().split("?")[0];
    if (trimmed.includes("/editor/"))
      return trimmed.split("/editor/").pop().split("?")[0];
    // Raw ID
    return trimmed;
  };
  ...
  const handlePreview = async () => {
    const sessionId = extractId(input);
    ...
    try {
      const session = await dispatch(previewSession(sessionId)).unwrap();

      setPStatus(session.isPublic ? "found" : "private");
    } catch {
      setPStatus("notfound");
    }
  };
  ...
  const handleJoin = async () => {
    ...
    try {
      await dispatch(joinSession(preview._id)).unwrap();
  ...
  const handleRequestAccess = async () => {
    ...
    try {
      ...
      await dispatch(requestAccess(preview._id)).unwrap();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Panel */}
      <div
        className="relative w-full max-w-md bg-[#0F0F11] border border-zinc-800
              ...
              <div className="mt-4">
                {requested ? (
                  <div
                    ...
                    "
                  >
                    Access request sent successfully. You'll be notified when
                    the owner responds.
                  </div>
                ) : (
                  <button
      ...
      "
          ...
          {pStatus === "found" && preview && (
            <div className="p-4 rounded-xl bg-zinc-900 border border-zinc-800 mb-5">
              ...
              {/* Meta row */}
              <div
                className="flex items-center gap-4 text-xs text-zinc-500 py-3
              ...
              >
                ...
                <span className="flex items-center gap-1.5">
                  {preview.owner?.avatar ? (
                    ...
                    <div
                      className="w-4 h-4 rounded-full bg-violet-700 flex items-center
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={handleJoin}
              disabled={pStatus !== "found" || joinLoading}
